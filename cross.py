...
class Cross:
    ...
    def find_cross(self, length: int) -> List[str]:
        ...
        return [
            (self.__convert_cross_numbers_to_notation([sol])[0], "cross")
    ...
    def __convert_cross_numbers_to_notation(
        self, notation_int: List[List[int]]
    ) -> List[str]:
        return [
            " ".join(convert.int_to_notation[x] for x in sol) for sol in notation_int
        ]
